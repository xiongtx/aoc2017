...
(defn parse-line
  [line]
  (let [[_ layer range] (re-find #"(\d+): (\d+)" line)]
    (mapv #(Long/parseLong %) [layer range])))
...
(defn init-state
  [firewall]
  (->> (interleave (keys firewall) (repeat 0))
       (apply hash-map)))

(defn move-scanners
  ([state]
   (move-scanners state 1))
  ([state m]
   (functor/fmap #(+ m %) state)))
...
(defn severity
  ([firewall]
   (let [layers (keys firewall)]
     (loop [i (inc (apply max layers))
            pos -1
            cost 0
            state (init-state firewall)]
       (if (zero? i)
         cost
         (let [new-pos (inc pos)
               new-cost (if-let [r (get firewall new-pos)]
                          (if (zero? (moves->pos r (get state new-pos)))
                            (+ cost (* new-pos r))
                            cost)
                          cost)
               new-state (move-scanners state)]
           (recur (dec i) new-pos new-cost new-state)))))))
...
(defn uncaught?
  [firewall delay]
  (->> firewall
       (map (fn [[layer range]] (moves->pos range (+ delay layer))))
       (not-any? zero?)))

(defn delay-time
  [firewall]
  ;; Avoid potential non-termination?
  (loop [i 0]
    (if (uncaught? firewall i)
      i
      (recur (inc i)))))
