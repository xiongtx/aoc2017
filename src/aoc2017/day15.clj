...
(defn generate
  ([multiplier seed]
   (->> (iterate #(mod (* % multiplier) 2147483647) seed)
        (drop 1)))
  ([multiplier seed n]
   (take n (generate multiplier seed))))

(def gen-a (partial generate 16807))

(def gen-b (partial generate 48271))
...
(defn num-equal
  [seed-a seed-b n]
  (->> (map vector (gen-a seed-a) (gen-b seed-b))
       (take n)
       (filter (fn [[a b]] (= (lower-16 a) (lower-16 b))))
       count))
...
(defn num-equal-picky
  [seed-a seed-b n]
  (->> (map vector
            (filter #(zero? (mod % 4)) (gen-a seed-a))
            (filter #(zero? (mod % 8)) (gen-b seed-b)))
       (take n)
       (filter (fn [[a b]] (= (lower-16 a) (lower-16 b))))
       count))
