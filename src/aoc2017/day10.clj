...
;; position moves past the end of the list, it wraps around to the
...
(defn reverse-segment
  ...
  (let [n (count circle)
        ...
        rs (->> (cycle circle)
                ...
                reverse)]
    (if (< k n)
      (concat (take pos circle) rs (drop k circle))
      (let [init (- n pos)
            wrap (- k n)
            middle (- pos wrap)]
        (concat (->> rs (drop init) (take wrap))
                (->> circle (drop wrap) (take middle))
                (take init rs))))))

(defn tie-knots
  ([lengths]
   (tie-knots lengths (range 256)))
  ([lengths circle]
   (tie-knots lengths circle 0))
  ([lengths circle pos]
   (tie-knots lengths circle pos 0))
  ([lengths circle pos skip]
   (let [n (count circle)]
     (reduce (fn [[circle pos skip] l]
               [(reverse-segment circle pos l)
                (mod (+ pos l skip) n)
                (inc skip)])
             [circle pos skip]
             lengths))))
...
(defn knot-hash
  ([lengths]
   (knot-hash lengths (range 256)))
  ([lengths circle]
   (->> (tie-knots lengths circle)
        first
        (take 2)
        (apply *))))
...
;; numbers are 64, 7, 255, they correspond to the hexadecimal numbers 40, 07,
;; ff, and so the first six characters of the hash would be 4007ff. Because
;; every Knot Hash is sixteen such numbers, the hexadecimal representation is
...
(defn parse-input-bytes
  [input]
  (concat (.getBytes (str/trim-newline input)) [17 31 73 47 23]))

(defn sparse-hash
  ([lengths]
   (loop [i 64
          [circle pos skip] [(range 256) 0 0]]
     (if (zero? i)
       circle
       (recur (dec i)
              (tie-knots lengths circle pos skip))))))
