...
;; pipes. Messages are passed between programs using these pipes, but most
...
;; intended recipient, and the programs suspect that some pipes are
...
(defn connect
  [uf [m ns]]
  (reduce #(uf/union %1 m %2) uf ns))

(defn build-uf
  [connections]
  (reduce connect (apply uf/union-find (map first connections)) connections))

(defn group-size
  [connections m]
  (let [es (map first connections)
        uf (build-uf connections)
        [_ root] (uf/get-canonical uf m)]
    (-> (filter #(= root (second (uf/get-canonical uf %))) es)
        count)))
