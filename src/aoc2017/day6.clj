...
;; removes all of the blocks from the selected bank, then moves to the
...
(defn most-blocks-index
  [bs]
  (.indexOf bs (apply max bs)))

(defn distribute
  [bs]
  (let [n (count bs)
        idx (most-blocks-index bs)]
    (loop [k (nth bs idx)
      ...
      (if (zero? k)
        ...
        (recur (dec k)
               (mod (inc next-idx) n)
               (update bs next-idx inc))))))

(defn num-until-cycle
  [bs]
  (let [orig-bs bs]
    (loop [i 1
           bs (distribute orig-bs)
           layouts #{orig-bs}]
      (if (contains? layouts bs)
        i
        (recur (inc i)
               (distribute bs)
               (conj layouts bs))))))
