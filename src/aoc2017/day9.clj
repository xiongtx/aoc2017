...
(defn score
  [input]
  (-> (reduce (fn [[score depth garbage? negate?] c]
                (if garbage?
                  (case c
                    \> [score depth negate? false]
                    \! [score depth true (not negate?)]
                    [score depth true false])
                  (case c
                    \{ [(+ score (inc depth)) (inc depth) garbage? false]
                    \} [score (dec depth) garbage? false]
                    \< [score depth true false]
                    [score depth false false])))
              [0 0 false false]
              input)
      first))
...
(defn num-garbage
  [input]
  (-> (reduce (fn [[k garbage? negate?] c]
                (if garbage?
                  (case c
                    \> [k negate? false]
                    \! [k true (not negate?)]
                    [(if negate? k (inc k)) true false])
                  [k (when (= \< c) true) false]))
              [0 false false]
              input)
      first))
